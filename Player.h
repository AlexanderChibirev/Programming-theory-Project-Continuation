...
public:
	...
	{
		if (STATE == stay) anim.set("stay");//если сидит больше 3ех сек то встает и не может сесть 3 секунды
		if  (STATE == run) anim.set("run");
		if (STATE == jump) { anim.set("jump"); }
		if (STATE == sit_down) { anim.set("sit_down"); Health += float(0.05);}
		...
		if (hit_on_enemy && dy == 0 &&STATE != dead_spike && shoot == false && STATE != sit_down) {
			timer += time;
			dx = 0;
			hit_on_enemy = false;
